...
def get_uncertain_point_coords_with_randomness(
    ...
    # print('point_coords', point_coords.shape) # point_coords torch.Size([1, 6144, 2])
    point_logits = point_sample(coarse_logits, point_coords, align_corners=False)
    # print('point_logits shape', point_logits.shape) # point_logits shape torch.Size([1, 9, 6144])
    ...
    # print('idx', idx.shape) # idx torch.Size([1, 2048])
...
def get_uncertain_point_coords_on_grid(uncertainty_map, num_points):
    ...
    """
    R, _, H, W = uncertainty_map.shape # (1, 1, 112, 80)
    h_step = 1.0 / float(H) # (1/112)
    w_step = 1.0 / float(W) # (1/80)

    num_points = min(H * W, num_points)
    point_indices = torch.topk(uncertainty_map.view(R, H * W), k=num_points, dim=1)[1] # 
    # print('point_indices', point_indices.shape) # point_indices torch.Size([1, 8192]) (0~112*80-1)
    
    point_coords = torch.zeros(R, num_points, 2, dtype=torch.float, device=uncertainty_map.device)
    point_coords[:, :, 0] = w_step / 2.0 + (point_indices % W).to(torch.float) * w_step
    point_coords[:, :, 1] = h_step / 2.0 + torch.div(point_indices, W, rounding_mode='floor').to(torch.float) * h_step
