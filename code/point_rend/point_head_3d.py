...
#             self.in_channels = 0
        if self.positional_encoding_enabled:
            self.in_channels += 128*2
            self.register_buffer("positional_encoding_gaussian_matrix", torch.randn((3, 128)))
            torch.nn.init.uniform_(self.positional_encoding_gaussian_matrix, a=0.0, b=1.0)
        assert self.in_channels > 0
        fc_dim_in = self.in_channels + self.num_classes
        self.fc_layers = nn.ModuleList()
        for k in range(self.num_layers):
            fc = nn.Conv1d(fc_dim_in, self.in_channels, kernel_size=1, stride=1, padding=0, bias=True)
            self.add_module("fc{}".format(k + 1), fc)
            self.fc_layers.append(fc)
            fc_dim_in = self.in_channels  
            fc_dim_in += self.num_classes if self.coarse_pred_each_layer else 0

        num_mask_classes = self.num_classes  
        self.predictor = nn.Conv1d(fc_dim_in, num_mask_classes, kernel_size=1, stride=1, padding=0)
        

    def forward(self, fine_grained_features, coarse_features, point_coords):
        # fine_grained_features: [R, channels, K]
        # coarse_features: [R, num_classes, K]
        # point_coords: [R, K, 2]
        ...
        if self.positional_encoding_enabled:
            # locations: [R*K, 2]
            locations = 2 * point_coords.reshape(num_instances * num_points, 3) - 1
            ...
            # print('locations', locations.shape)
            # locations: [R, C, K]
            locations = locations.reshape(num_instances, num_points, 128*2).permute(0, 2, 1)
            # print('location', locations.shape)
            # print('fine_granded', fine_grained_features.shape)
        ...
        # print('fine_grained_features shape', fine_grained_features.shape) # fine_grained_features shape torch.Size([4, 512, 2048])
        # print('coarse_features', coarse_features.shape) # coarse_features torch.Size([4, 9, 2048])
        fine_grained_features = torch.cat([fine_grained_features, coarse_features], dim=1)
        x = fine_grained_features # fine_grained_features shape torch.Size([4, 512, 2048])
        for layer in self.fc_layers:
            # print('x', x.shape)
            x = F.relu(layer(x))
            # print('internal shape', x.shape) # internal shape torch.Size([4, 256, 2048])
            if self.coarse_pred_each_layer:
                x = cat((x, coarse_features), dim=1) # 
        return self.predictor(x)
...
#     return head_name(cfg, input_channels)
...
class StandardPointHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        super(StandardPointHead, self).__init__()
        # fmt: off
        num_classes                 = cfg.num_classes                                  # 9
        fc_dim                      = cfg.fc_dim                     # 256
        num_fc                      = cfg.num_fc                      # 3
        cls_agnostic_mask           = cfg.cls_agnostic_mask           # False
        self.coarse_pred_each_layer = cfg.coarse_pred_each_layer      # True
        input_channels              = cfg.input_channels                               # 256
        # fmt: on
        ...
        num_mask_classes = input_channels if cls_agnostic_mask else num_classes
        self.predictor = nn.Conv1d(fc_dim_in, num_mask_classes, kernel_size=1, stride=1, padding=0)
...
if __name__ == '__main__':
    ...
    # model = StandardPointHead(args)
    fine_grained_features = torch.randn(4, 256, 2048)
    coarse_features = torch.randn(4, 9, 2048)
    coords = torch.ones(4, 2048, 2)
    
    
    # res = model(fine_grained_features, coarse_features)
    # print(res.shape) # torch.Size([4, 9, 2048])
    model = ImplicitPointHead(args)
    res = model(fine_grained_features, coarse_features, coords)
    # print(res.shape) # torch.Size([4, 9, 2048])
