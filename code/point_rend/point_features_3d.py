...
def get_uncertain_point_coords_with_randomness(
    ...
    # print('point_coords', point_coords.shape) # point_coords torch.Size([1, 6144, 1, 3])
    point_logits = point_sample(coarse_logits, point_coords, align_corners=False)
    # print('point_logits shape', point_logits.shape) # point_logits shape torch.Size([1, 2, 6144, 1, 1])
    ...
    # print('idx', idx.shape) # idx torch.Size([1, 2048, 1, 1])
...
def get_uncertain_point_coords_on_grid(uncertainty_map, num_points):
    ...
    # print(uncertainty_map.shape) # torch.Size([1, 1, 224, 224, 160])
    d_step = 1.0 / float(D)
    h_step = 1.0 / float(H)
    w_step = 1.0 / float(W)
    ...
    d = torch.div(point_indices, (W*H), rounding_mode='floor').to(torch.float)
    point_indices = point_indices % (W*H)
    
    w = (point_indices % W).to(torch.float)
    h = torch.div(point_indices, W, rounding_mode='floor').to(torch.float) 
    
    # print(d.shape)
    point_coords[:, :, 2] = d*d_step + d_step/2
    point_coords[:, :, 1] = h*h_step + h_step/2
    point_coords[:, :, 0] = w*w_step + w_step/2
    
    # point_coords[:, :, 0] = d_step / 2.0 + torch.div(point_indices, W*H, rounding_mode='floor').to(torch.float) * d_step
    # point_indices = torch.div(point_indices, W*H, rounding_mode='floor')
    # point_coords[:, :, 1] = w_step / 2.0 + torch.div(point_indices, W, rounding_mode='floor').to(torch.float) * h_step
    # point_coords[:, :, 1] = w_step / 2.0 + (point_indices % W).to(torch.float) * w_step
    # point_coords[:, :, 2] = h_step / 2.0 + torch.div(point_indices, W, rounding_mode='floor').to(torch.float) * d_step
