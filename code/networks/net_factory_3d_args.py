...
from networks.vnetWithArgs import VNet
...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        # assert isinstance(self.encoders, nn.ModuleList)
        # create decoder path
        self.decoders = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                                        upsample=True)
        # assert isinstance(self.decoders, nn.ModuleList)
...
if __name__ == '__main__':
    model = UNet3D(in_channels=1, out_channels=9)
    inp = torch.randn(2, 1, 112, 112, 80)
    res, feat = model(inp)
    print(res.shape)
    print([item.shape for item in feat])