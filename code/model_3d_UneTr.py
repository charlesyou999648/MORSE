...
def create_model_3d(num_classes=4):
    ...
    #                     class_num=num_classes)
    model = UNETR(in_channels=1, out_channels=num_classes)
    return model
...
class MOEHead_3d(nn.Module):
    def __init__(self, fea_dim=[128, 64, 32, 16, 16], output_dim=128, use_sparse=False) -> None:
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.use_sparse and self.training and random.random()<0.1:
...
class PointRend(nn.Module):
    def __init__(self, args) -> None:
        ...
        if args.moe:
            self.feature_extractor = MOEHead_3d_UseOne(fea_dim=[64, 32, 16], \
                output_dim=args.in_features, use_sparse=args.moe_random)
        else:
            self.feature_extractor = FeatureExtractor_3d(fea_dim=[64, 32, 16], \
...
class Patcher_UseOne(nn.Module):
    def __init__(self, args) -> None:
        super().__init__()
        self.model = create_model_3d(num_classes=args.num_classes)
        self.feature_extractor = MOEHead_3d_UseOne(fea_dim=[64, 64, 64], \
...
class Patcher(nn.Module):
    def __init__(self, args) -> None:
        super().__init__()
        self.model = create_model_3d(num_classes=args.num_classes)
        self.feature_extractor = MOEHead_3d(fea_dim=[256, 128, 64], \
            output_dim=args.num_classes, use_sparse=args.moe_random)
    
    def forward(self, image, label=None):
        _, featmap = self.model(image)
        res = self.feature_extractor(featmap[-3:])
        return res, featmap
...
class PointRend_patcher(nn.Module):
    def __init__(self, args) -> None:
        ...
        self.feature_extractor = FeatureExtractor_3d(fea_dim=[256, 128, 64], output_dim=args.in_features)
        self.pointrend_seg_head = PointRendSemSegHead(args)
